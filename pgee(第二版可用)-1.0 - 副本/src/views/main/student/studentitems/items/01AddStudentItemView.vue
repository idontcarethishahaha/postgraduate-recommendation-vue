...
import { createElNotificationError } from '@/components/message'
...
const rootItemIdR = toRef(() => {
  const itemid = route.params.itemid as string | undefined
  return itemid || '' // 无参数时返回空字符串
})
const fetchR = ref(false)

const studentItemsQuery = StudentService.listStudentItemsService(rootItemIdR)
const rootItemQuery = StudentService.listItemsService(rootItemIdR, fetchR)
const { data: studentItemsR, suspense: stuItemSusp } = studentItemsQuery
const { data: rootItemR, suspense: itemsSusp } = rootItemQuery
...
// 第一级及以下
const activeF = async () => {
  if (!rootItemIdR.value) {
    createElNotificationError('请先选择具体的指标项分类！')
    return
  }
  ...
  try {
    ...
    createElNotificationError('加载指标项数据失败，请重试！')
...
const selectItemCallback = (item: Item) => {
  ...
  if (!rootItem?.items || !item.id) return
...
<template>
  <el-button type="primary" @click="activeF" :disabled="!rootItemIdR">
  ...
  <el-dialog v-model="dialogVisible" title="添加项" @close="closeF">
    ...
    <div v-else>
      <el-empty description="暂无指标项数据"></el-empty>
    </div>

    <el-form v-if="dialogVisible && rootItemR">
