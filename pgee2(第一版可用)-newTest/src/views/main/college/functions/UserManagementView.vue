...
const submitF = async () => {
  try {
    ...
    createElNotificationError('添加失败，请稍后重试')
  }
}
...
const submitC = computed(() => {
  ...
  return !!userR.value.name && !!userR.value.account && !!userR.value.majorCategoryId && telValid
})

// 管理辅导员相关
const activeCategory = ref<{ id: string; name: string } | null>(null) // 当前激活的类别
